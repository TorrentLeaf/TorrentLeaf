-- Add 'video' to the file_type check constraint on torrent_files
-- and to the content_type check constraint on library_items.
...
ALTER TABLE torrent_files
  ...
  CHECK (file_type IN ('image', 'pdf', 'epub', 'cbz', 'cbr', 'video', 'unknown'));
...
ALTER TABLE library_items
  ...
  CHECK (content_type IN ('manga', 'book', 'document', 'video', 'other'));
